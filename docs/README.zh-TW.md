...
- **ActivityWatch** — 開源、隱私優先，功能上挑不出毛病。但老實說，它的介面沒什麼吸引力，裝完打開看過一次，之後就再也沒點開過。
- **WorkReview 這類工具** — 我想要兩件事同時滿足：一是能跨裝置彙總，二是像 iPhone「螢幕使用時間」那樣按小時縮放的時間軸，讓我直接看到「下午 3 點我在做什麼」。桌面端沒有一款做到讓我滿意。
- **Toggl / RescueTime / 各種付費 SaaS** — 這些本質上是給團隊和 HR 算「計費工時」用的：儀表板資訊密集，流程繞著專案貼標籤轉，資料還要傳到對方的雲端。我要的是「自己跟自己覆盤」，方向完全對不上。
